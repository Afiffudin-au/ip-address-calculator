...
function calculate(ip, prefix) {
  ...
  if (prefix >= 24 && prefix <= 32) {
    ...
    calculation_text[0].innerHTML = (`<td>Calculate IP</td> <td>${selectArray} / ${ClassIP[`__${prefix}`]} = ${Math.floor(selectArray / ClassIP[`__${prefix}`])} * ${ClassIP[`__${prefix}`]} = ${Math.floor(selectArray / ClassIP[`__${prefix}`]) * ClassIP[`__${prefix}`]}</td>`)
    ...
    calculation_text[1].innerHTML = (`<td>IP firstResult</td> <td>${afterSplit.join('.')} 
    => ${firstIPArray.join('.')} </td>`)
    calculation_text[2].innerHTML = (`<td>Calculate IP</td>  <td>${firstIP} + ${ClassIP[`__${prefix}`]} - 1 = ${firstIP + ClassIP[`__${prefix}`] - 1}</td>`)
    copyArray2[3] = lastIP
    lastIPArray = (copyArray2)
    calculation_text[3].innerHTML = (`<td>IP lastResult</td> <td>${afterSplit.join('.')} 
    => ${lastIPArray.join('.')} </td>`)
    calculation_text[4].innerHTML = (`<td>NETWORK ID (firstResult)</td><td>${firstIPArray.join('.')}</td>`)
    calculation_text[5].innerHTML = (`<td>BROADCAST (lastResult)</td><td>${lastIPArray.join('.')}</td>`)
    subnetMask = 256 - ClassIP[`__${prefix}`]
    calculation_text[6].innerHTML = (`<td>Subnet calculate</td> <td>256 - ${ClassIP[`__${prefix}`]} = ${256 - ClassIP[`__${prefix}`]}</td>`)
    calculation_text[7].innerHTML = (`<td>Subnet result</td> <td>255.255.255.${subnetMask}</td>`)
    ...
    calculation_text[8].innerHTML = (`<td>RANGE IP </td> <td>${firstIPArray.join('.')} - ${lastIPArray.join('.')}</td>`)
    calculation_text[9].innerHTML = (`<td>VALID HOST Range</td><td>${usableHostIpRange.first.join('.')} - ${usableHostIpRange.last.join('.')}</td>`)
    calculation_text[10].innerHTML = (`<td>Valid Host</td> <td>${(ClassIP[`__${prefix}`])} - 2 = ${(ClassIP[`__${prefix}`] - 2)}</td>`)
    const results = (
      `
      <h3>Hasil</h3>
      <tr>
  ...
  if (prefix >= 16 && prefix <= 23) {
    ...
    calculation_text[0].innerHTML = (`<td>imaginer</td><td>${prefix} + 8 = ${prefixInt + 8}</td>`)
    const IPclass = ClassIP[`__${imaginer}`]
    const totalIP = IPclass * 256
    calculation_text[1].innerHTML = (`<td>total IP </td> <td>  ${IPclass} * 256 = ${IPclass * 256}</td>`)
    const firstIP = Math.floor(selectArray2 / IPclass) * IPclass
    calculation_text[2].innerHTML = (`<td>Calculate IP </td> <td> ${selectArray2} / ${IPclass} = ${Math.floor(selectArray2 / IPclass)} * ${IPclass} = ${Math.floor(selectArray2 / IPclass) * IPclass}</td>`)
    const lastIP = firstIP + IPclass - 1
    const copyArray = [...afterSplit]
    copyArray[3] = 0
    copyArray[2] = firstIP
    firstIPArray = copyArray
    calculation_text[3].innerHTML = (`<td>IP firstResult</td> <td>${afterSplit.join('.')} => ${firstIPArray.join('.')}</td>`)
    calculation_text[4].innerHTML = (`<td>Calculate IP</td> <td>${firstIP} + ${IPclass} - 1 = ${firstIP + IPclass - 1}</td>`)
    const copyArray2 = [...afterSplit]
    copyArray2[3] = 255
    copyArray2[2] = lastIP
    lastIPArray = copyArray2
    calculation_text[5].innerHTML = (`<td>IP lastResult</td> <td>${afterSplit.join('.')} => ${lastIPArray.join('.')}</td>`)
    calculation_text[6].innerHTML = (`<td>NETWORK ID (firstResult) </td> <td>${firstIPArray.join('.')}</td>`)
    calculation_text[7].innerHTML = (`<td>BROADCAST (lastResult)</td>  <td>${lastIPArray.join('.')}</td>`)
    subnetMask = 256 - IPclass
    calculation_text[8].innerHTML = (`<td>Subnet calculate</td>  <td>256 - ${IPclass} = ${256 - IPclass}</td>`)
    calculation_text[9].innerHTML = (`<td>SUBNET MASK </td> <td>255.255.${subnetMask}.0</td>`)
    const copyArray3 = [...firstIPArray]
    copyArray3[3] = copyArray3[3] + 1
    usableHostIpRange.first = copyArray3
    const copyArray4 = [...lastIPArray]
    copyArray4[3] = copyArray4[3] - 1
    usableHostIpRange.last = copyArray4
    calculation_text[10].innerHTML = (`<td>RANGE IP </td> <td>${firstIPArray.join('.')} - ${lastIPArray.join('.')}</td>`)
    calculation_text[11].innerHTML = (`<td>VALID HOST Range</td> <td>${usableHostIpRange.first.join('.')} - ${usableHostIpRange.last.join('.')}</td>`)
    calculation_text[12].innerHTML = (`<td>Valid Host </td> <td>${(totalIP)} - 2 = ${(totalIP - 2)}</td>`)
    const results = (
      `
    <h3>Hasil</h3>
    <tr>
